...
from backend.core.engagement.engagement_manager import EngagementManager, ValidationError

router = APIRouter()


def _eng_manager_provider(request: Request):
    provided: DIContainer.Provided = request.app.state.di_provided
    repo = EngagementRepository(provided.engagement_store)
    return EngagementManager(repo)


@router.post("/", response_model=EngagementResponse, status_code=status.HTTP_201_CREATED)
def create_engagement(req: EngagementCreateRequest, manager=Depends(_eng_manager_provider)) -> EngagementResponse:
    try:
...
def get_engagement(eng_id: str, manager=Depends(_eng_manager_provider)) -> EngagementResponse:
    try:
...
def update_engagement(eng_id: str, req: EngagementUpdateRequest, manager=Depends(_eng_manager_provider)) -> EngagementResponse:
    try:
